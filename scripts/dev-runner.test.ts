...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("createDevRunnerEnv", () => {
    it.effect(
      "defaults T3CODE_HOME to workspace-local .t3-dev when not provided",
      () =>
        Effect.gen(function* () {
          ...
          assert.equal(
            env.T3CODE_HOME,
            path.resolve(import.meta.dirname, "..", ".t3-dev"),
          );
          assert.equal(
          ...
          assert.equal(
            env.GC_CITY_PATH,
            getBundledGascityConfigLayout("gascity-br").rootDir,
          );
        }),
    );
    ...
    it.effect("defaults dev server mode to the upstream backend port", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.VITE_WS_URL, "ws://localhost:3773");
      }),
    );
  });
  ...
  describe("findFirstAvailableOffset", () => {
    ...
    it.effect("advances until all required ports are available", () =>
      Effect.gen(function* () {
        const taken = new Set([3773, 5733, 13774, 5734]);
        const offset = yield* findFirstAvailableOffset({
          startOffset: 0,
          requireServerPort: true,
          requireWebPort: true,
