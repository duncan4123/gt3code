...
```bash
WISP=$(bd mol wisp mol-jj-lander-patrol --root-only --var target=integration/t3code-agent-controls-sidebar --var target_remote=jorje --var binding_prefix={{ .BindingPrefix }} --json | jq -r '.new_epic_id')
