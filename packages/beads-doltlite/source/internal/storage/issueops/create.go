...
func PersistLabels(ctx context.Context, tx *sql.Tx, issue *types.Issue, actor, eventTable string) (CreateIssueResult, error) {
	return PersistLabelsWithDialect(ctx, tx, issue, actor, eventTable, SQLDialectDolt)
}

func PersistLabelsWithDialect(ctx context.Context, tx *sql.Tx, issue *types.Issue, actor, eventTable string, dialect SQLDialect) (CreateIssueResult, error) {
	var result CreateIssueResult
	...
	for _, label := range issue.Labels {
		...
		sqlResult, err := tx.ExecContext(ctx, fmt.Sprintf(insert, labelTable), issue.ID, label)
		if err != nil {
		...
		if err := RecordEventInTableWithDialect(ctx, tx, eventTable, issue.ID, types.EventLabelAdded, actor, comment, dialect); err != nil {
