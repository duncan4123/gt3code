...
Tcl tests are stored in files that match the pattern _\*.test_. They are
found in both the $TOP/test/ directory, and in the various sub-directories
of the $TOP/ext/ directory of the source tree. Not all _\*.test_ files
...
File _permutations.test_ defines various test "permutations". A permutation
consists of:
...
are defined in file _testrunner_data.tcl_.
...
be retrieved from either _testrunner.log_ or _testrunner.db_.
...
on Linux, Windows or OSX. Refer to _testrunner_data.tcl_ for the details
...
this, simply add the "`--fuzzdb` _FILENAME_" command-line option or set
...
databases. So if you want to run _only_ tests involving the external
