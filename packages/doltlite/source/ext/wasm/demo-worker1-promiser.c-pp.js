...
(async function () {
  ...
  const wtest = async function (msgType, msgArgs, callback) {
    ...
    const p = 1 ? workerPromise({ type: msgType, args: msgArgs }) : workerPromise(msgType, msgArgs);
  ...
  const runTests = async function () {
    ...
    logHtml("", "Sending 'open' message and waiting for its response before continuing...");
  ...
  const runTests2 = async function () {
    ...
    await wtest(
      "exec",
      {
        sql: ["create table t(a,b)", "insert into t(a,b) values(1,2),(3,4),(5,6)"].join(";"),
        resultRows: [],
        columnNames: [],
      ...
      function (ev) {
        ...
        T.assert(0 === ev.resultRows.length)
          ...
          .assert(sqConfig.bigIntEnabled ? 3n === ev.changeCount : 3 === ev.changeCount)
    ...
    await wtest("exec", { sql: "intentional_error" }, mustNotReach).catch((e) => {
