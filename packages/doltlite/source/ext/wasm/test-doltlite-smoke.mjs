...
if (nodeEntryText.includes("__dirname + '/'")) {
  nodeEntryText = nodeEntryText.replaceAll("__dirname + '/'", "new URL('.', import.meta.url).href");
...
const dbFile = path.join(fs.mkdtempSync(path.join(os.tmpdir(), "doltlite-wasm-")), "smoke.db");
