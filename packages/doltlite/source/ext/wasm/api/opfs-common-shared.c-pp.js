...
globalThis.sqlite3ApiBootstrap.initializers.push(
  function (sqlite3) {
    "use strict";
    if (sqlite3.config.disable?.vfs?.opfs && sqlite3.config.disable.vfs["opfs-vfs"]) {
      return;
    }
  ...
  */
    opfsUtil.getRootDir = async function f() {
      return (f.promise ??= navigator.storage
        .getDirectory()
        .then((d) => {
      ...
      if (!opfsUtil.proxyUri) {
        ...
        if (sqlite3.scriptInfo?.sqlite3Dir) {
          /* Doing this from one scope up, outside of this function, does
           not work. */
          opfsUtil.proxyUri = sqlite3.scriptInfo.sqlite3Dir + opfsUtil.proxyUri;
        }
      }
    ...
    */
      opfsVfs.bindVfs = function (ioMethods, callback) {
        ...
        const thePromise = new Promise(function (promiseResolve_, promiseReject_) {
          ...
          const proxyUri =
            options.proxyUri + (options.proxyUri.indexOf("?") < 0 ? "?" : "&") + "vfs=" + vfsName;
