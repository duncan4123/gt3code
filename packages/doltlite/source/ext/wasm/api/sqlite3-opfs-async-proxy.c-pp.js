  ...
  */
  ...
  if (isWebLocker) {
    ...
    */
    ...
    vfsAsyncImpls.xLock = async function (
      fid /*sqlite3_file pointer*/,
      ...
      const requestedMode =
        lockType >= state.sq3Codes.SQLITE_LOCK_RESERVED ? "exclusive" : "shared";
      const existing = __activeWebLocks[fid];
      if (existing) {
        if (
          existing.mode === requestedMode ||
          (existing.mode === "exclusive" && requestedMode === "shared")
        ) {
          fh.xLock = lockType;
    ...
    vfsAsyncImpls.xUnlock = async function (
      fid /*sqlite3_file pointer*/,
      lockType /*SQLITE_LOCK_...*/,
    ) {
      const fh = __openFiles[fid];
    ...
    while (!flagAsyncShutdown) {
      try {
        ...
        if (hasWaitAsync) {
          ...
          */
          if (
            "not-equal" !== Atomics.wait(state.sabOPView, slotWhichOp, 0, state.asyncIdleWaitTime)
          ) {
