...
globalThis.sqlite3ApiBootstrap = async function sqlite3ApiBootstrap(
  ...
  */
  ...
  /** Internal helper for SQLite3Error ctor. */
  const isInt32 = (n) =>
    "number" === typeof n && n === (n | 0) && n <= 2147483647 && n >= -2147483648;
  ...
  */
  ...
  const affirmBindableTypedArray = (v) =>
    isBindableTypedArray(v) || toss3("Value is not of a supported TypedArray type.");
  ...
  */
  const flexibleString = function (v) {
    if (isSQLableTypedArray(v)) {
      return wasm.typedArrayToString(v instanceof ArrayBuffer ? new Uint8Array(v) : v, 0, v.length);
  ...
  */
  ...
  Object.assign(
    ...
    */
      sqlite3_prepare_v2: (dbPtr, sql, sqlByteLen, stmtPtrPtr, strPtrPtr) => {} /*installed later*/,
  ...
  capi.sqlite3_result_error_js = function (pCtx, e) {
    if (e instanceof WasmAllocError) {
      ...
      /* Maintenance reminder: ''+e, rather than e.message,
         ...
         the exception's type name in the result. */ capi.sqlite3_result_error(pCtx, "" + e, -1);
    }
  };
        ...
        if (capi.SQLITE_BUSY === rc) {
          if (n === maxTimes) {
            throw new SQLite3Error(
              rc,
              ["sqlite3_js_retry_busy() max retry attempts (", maxTimes, ") reached."].join(""),
            );
          }
          continue;
        }
