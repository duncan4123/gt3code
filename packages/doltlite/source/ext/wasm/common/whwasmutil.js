...
globalThis.WhWasmUtilInstaller = function WhWasmUtilInstaller(target) {
  ...
  */
  ...
  /** Set up target.ptr... */
  {
    ...
    (function f(name, val) {
      ...
      return f;
    })("null", __NullPtr)("size", __ptrSize)("ir", __ptrIR)("coerce", __asPtrType)("add", __ptrAdd)(
      "addn",
  ...
  */
  target.heapForSize = function (n, unsigned = true) {
    let ctor;
    const c =
      cache.memory && cache.heapSize === cache.memory.buffer.byteLength ? cache : heapWrappers();
    switch (n) {
      case Int8Array:
        return c.HEAP8;
      case Uint8Array:
  ...
  target.peek = function f(ptr, type = "i8") {
    ...
    do {
      ...
      switch (type) {
        case "i1":
        case "i8":
          rc = c.HEAP8[Number(ptr /*tag:64bit*/) >> 0];
          break;
        case "i16":
          rc = c.HEAP16[Number(ptr /*tag:64bit*/) >> 1];
          break;
        case "i32":
          rc = c.HEAP32[Number(ptr /*tag:64bit*/) >> 2];
          break;
        case "float":
        case "f32":
          rc = c.HEAP32F[Number(ptr /*tag:64bit*/) >> 2];
          break;
        case "double":
        case "f64":
          rc = Number(c.HEAP64F[Number(ptr /*tag:64bit*/) >> 3]);
          break;
        case "i64":
          if (c.HEAP64) {
  ...
  target.poke = function (ptr, value, type = "i8") {
    ...
    for (const p of Array.isArray(ptr) ? ptr : [ptr]) {
      switch (type) {
        case "i1":
        case "i8":
          c.HEAP8[Number(p /*tag:64bit*/) >> 0] = value;
          continue;
        case "i16":
          c.HEAP16[Number(p /*tag:64bit*/) >> 1] = value;
          continue;
        case "i32":
          c.HEAP32[Number(p /*tag:64bit*/) >> 2] = value;
          continue;
        case "float":
        case "f32":
          c.HEAP32F[Number(p /*tag:64bit*/) >> 2] = value;
          continue;
        case "double":
        case "f64":
          c.HEAP64F[Number(p /*tag:64bit*/) >> 3] = value;
          continue;
        case "i64":
          if (c.HEAP64) {
  ...
  */
  target.cstrncpy = function (tgtPtr, srcPtr, n) {
    ...
    const tgtNumber = Number(tgtPtr),
      srcNumber = Number(srcPtr) /*tag:64bit*/;
  ...
  */
  ...
  for (const t of ["i8", "i16", "i32", "i64", "int", "f32", "float", "f64", "double"]) {
    ...
    convertArg(v, argv, argIndex) {
      ...
      if (v instanceof Function) {
        ...
        if (pair && pair[1] && pair[1] !== v) {
          ...
          try {
            cache.scopedAlloc.pushPtr(pair[1]); /*see notes above*/
          } catch (e) {
            /*ignored*/
          }
  ...
  */
  target.xWrap = function callee(fArg, resultType, ...argTypes) {
    ...
    if (target.isPtr(fArg)) {
      fArg =
        target.functionEntry(fArg) || toss("Function pointer not found in WASM function table.");
    }
    ...
    /*Verify the arg type conversions are valid...*/ __xResultAdapterCheck(resultType);
...
globalThis.WhWasmUtilInstaller.yawl = function yawl(config) {
  ...
  const loadWasm = WebAssembly.instantiateStreaming
    ? () => WebAssembly.instantiateStreaming(wfetch(), config.imports || {}).then(finalThen)
    : () =>
        wfetch() // Safari < v15
