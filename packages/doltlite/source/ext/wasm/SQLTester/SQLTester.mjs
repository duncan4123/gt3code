...
const __SAB =
  "undefined" === typeof globalThis.SharedArrayBuffer
    ? function () {}
...
const Util = newObj({
  ...
  unlink: function f(fn) {
    if (!f.unlink) {
      f.unlink = sqlite3.wasm.xWrap("sqlite3__wasm_vfs_unlink", "int", ["*", "string"]);
    }
  ...
  strglob: sqlite3.wasm.xWrap("sqlite3__wasm_SQLTester_strglob", "int", ["string", "string"]),
}) /*Util*/;
...
class Outer {
  ...
  outputPrefix() {
    if (0 == arguments.length) {
      return this.getOutputPrefix ? (this.getOutputPrefix() ?? "") : "";
    } else {
  ...
  verbose1(...args) {
    return this.verboseN(1, args);
  }
  verbose2(...args) {
    return this.verboseN(2, args);
  }
  verbose3(...args) {
    return this.verboseN(3, args);
  }
...
class SQLTester {
  ...
  outln(...args) {
    return this.#outer.outln(...args);
  }
  out(...args) {
    return this.#outer.out(...args);
  }
  outer(...args) {
    if (args.length) {
      this.#outer = args[0];
  ...
  verbose1(...args) {
    return this.#outer.verboseN(1, args);
  }
  verbose2(...args) {
    return this.#outer.verboseN(2, args);
  }
  verbose3(...args) {
    return this.#outer.verboseN(3, args);
  }
  verbosity(...args) {
  ...
  clearInputBuffer() {
    return this.#clearBuffer(this.#inputBuffer);
  }
  clearResultBuffer() {
    return this.#clearBuffer(this.#resultBuffer);
  }
  ...
  nullValue() {
    return 0 == arguments.length ? this.#nullView : (this.#nullView = "" + arguments[0]);
  }

  outputColumnNames() {
    return 0 == arguments.length ? this.#emitColNames : (this.#emitColNames = !!arguments[0]);
  }
  ...
  getCurrentDb() {
    return this.#db.list[this.#db.iCurrentDb];
  }
    ...
    try {
      ...
      if (0 != rc) {
        sqlite3.SQLite3Error.toss(
          ...
          "sqlite3 result code",
          rc + ":",
          pDb ? sqlite3.capi.sqlite3_errmsg(pDb) : sqlite3.capi.sqlite3_errstr(rc),
        );
      }
  ...
  #setupInitialDb() {
    if (!this.#db.list[0]) {
      ...
      this.#outer.outln(
        "WARNING: setupInitialDb() was unexpectedly ",
  ...
  execSql(pDb, throwOnError, appendMode, rowMode, sql) {
    ...
    const wasm = sqlite3.wasm,
      capi = sqlite3.capi;
    sql = sql instanceof Uint8Array ? sql : Util.utf8Encode(capi.sqlite3_js_sql_to_string(sql));
    const self = this;
    ...
    wasm.scopedAllocCall(function () {
      ...
      while (pSql && wasm.peek8(pSql)) {
        ...
        rc = capi.sqlite3_prepare_v3(pDb, pSql, sqlByteLen, 0, ppStmt, pzTail);
        if (0 !== rc) {
          if (throwOnError) {
        ...
        if (sb) {
          ...
          // no output but possibly other side effects
          while (capi.SQLITE_ROW === (rc = capi.sqlite3_step(pStmt))) {}
        }
    ...
    }) /* SQL script loop */ /*scopedAllocCall()*/;
    return rc;
  }
} /*SQLTester*/
...
class TestScript {
  ...
  constructor(...args) {
    ...
    if (!(content instanceof Uint8Array)) {
      ...
      } else if (content instanceof ArrayBuffer || content instanceof Array) {
  ...
  moduleName() {
    return 0 == arguments.length ? this.#moduleName : (this.#moduleName = arguments[0]);
  }

  testCaseName() {
    return 0 == arguments.length ? this.#testCaseName : (this.#testCaseName = arguments[0]);
  }
  filename() {
    return 0 == arguments.length ? this.#filename : (this.#filename = arguments[0]);
  }
      ...
      switch (b) {
        ...
        default: {
          ...
          if (1 == nChar) {
            ...
            cur.sb.push(Util.utf8Decode(new Uint8Array(aChar)));
            i += nChar - 1;
          }
          break;
        }
...
class GlobCommand extends Command {
  ...
  process(t, ts, argv) {
    ...
    let rc = t.execSql(null, true, ResultBufferMode.ESCAPED, ResultRowMode.ONELINE, sql);
...
class ResultCommand extends Command {
  ...
  process(t, ts, argv) {
    ...
    if (result !== sArgs) {
      t.outln(
        argv[0],
        " FAILED comparison. Result buffer:\n",
        result,
        "\nExpected result:\n",
        sArgs,
      );
...
class RunCommand extends Command {
  process(t, ts, argv) {
    this.argcCheck(ts, argv, 0, 1);
    const pDb = 1 == argv.length ? t.currentDb() : t.getDbById(parseInt(argv[1]));
    const sql = t.takeInputBuffer();
    const rc = t.execSql(pDb, false, ResultBufferMode.NONE, ResultRowMode.ONELINE, sql);
    if (0 !== rc && t.verbosity() > 0) {
      const msg = sqlite3.capi.sqlite3_errmsg(pDb);
      ts.verbose2(argv[0], " non-fatal command error #", rc, ": ", msg, "\nfor SQL:\n", sql);
    }
  }
}
...
class TableResultCommand extends Command {
  ...
  process(t, ts, argv) {
    ...
    if (globs.length < 1) {
      ts.toss(argv[0], " requires 1 or more ", this.#jsonMode ? "json snippets" : "globs", ".");
    }
    const sql = t.takeInputBuffer();
    t.execSql(
      null,
      ...
      ResultRowMode.NEWLINE,
      sql,
    );
    const rbuf = t.getResultText().trim();
    const res = rbuf.split(/\r?\n/);
    if (res.length !== globs.length) {
      ts.toss(argv[0], " failure: input has ", res.length, " row(s) but expecting ", globs.length);
    }
    for (let i = 0; i < res.length; ++i) {
      ...
      if (this.#jsonMode) {
        if (glob !== res[i]) {
          ts.toss(argv[0], " json <<", glob, ">> does not match: <<", res[i], ">>");
        }
...
class CommandDispatcher {
  ...
  static getCommandByName(name) {
    ...
    switch (name) {
      case "close":
        rv = new CloseDbCommand();
        break;
      case "column-names":
        rv = new ColumnNamesCommand();
        break;
      case "db":
        rv = new DbCommand();
        break;
      ...
      case "json":
        ...
        break;
      case "json-block":
        rv = new JsonBlockCommand();
        break;
      case "new":
        rv = new NewDbCommand();
        break;
      case "notglob":
        rv = new NotGlobCommand();
        break;
      case "null":
        rv = new NullCommand();
        break;
      case "oom":
        ...
        break;
      case "open":
        rv = new OpenDbCommand();
        break;
      case "print":
        rv = new PrintCommand();
        break;
      case "result":
        rv = new ResultCommand();
        break;
      case "run":
        rv = new RunCommand();
        break;
      case "tableresult":
        rv = new TableResultCommand();
        break;
      case "testcase":
        rv = new TestCaseCommand();
        break;
      case "verbosity":
        rv = new VerbosityCommand();
        break;
    }
    if (rv) {
