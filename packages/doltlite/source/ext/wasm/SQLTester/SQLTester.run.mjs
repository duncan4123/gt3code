...
const out = function f(...args) {
  return f.outer.out(...args);
};
out.outer = new ns.Outer();
out.outer.getOutputPrefix = () => "SQLTester.run: ";
const outln = (...args) => {
  return out.outer.outln(...args);
};

const affirm = function (expr, msg) {
  if (!expr) {
    throw new Error(arguments[1] ? "Assertion failed: " + arguments[1] : "Assertion failed");
  }
};

let ts = new ns.TestScript(
  "SQLTester-sanity-check.test",
  `
/*
...
const sqt = new ns.SQLTester().setLogger(console.log.bind(console)).verbosity(1).addTestScript(ts);
