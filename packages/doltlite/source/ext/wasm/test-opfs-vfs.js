...
const tryOpfsVfs = async function (sqlite3) {
  ...
  try {
    ...
    db.transaction((db) => {
      db.exec({
        sql: ["create table if not exists t(a);", "insert into t(a) values(?),(?),(?);"],
        bind: [performance.now() | 0, (performance.now() | 0) / 2, (performance.now() | 0) / 4],
      });
    });
    ...
    (await opfs.unlink(testDir + "/test/dir")) &&
      toss("delete 2b should have failed (dir already deleted)");
