...
(function () {
  ...
  */
  ...
  const runSpeedtest = async function (cliFlagsArray) {
    ...
    try {
      const argv = ["speedtest1.wasm", ...cliFlagsArray, dbFile];
      ...
      if (App.sqlite3.installOpfsSAHPoolVfs && !App.sqlite3.$SAHPoolUtil && ndxSahPool > 0) {
      ...
      App.wasm.xCall("wasm_main", argv.length, App.wasm.scopedAllocMainArgv(argv));
  ...
  globalThis.onmessage = function (msg) {
    ...
    switch (msg.type) {
      case "run":
        runSpeedtest(msg.data || []).catch((e) => mPost("error", e));
        break;
      default:
