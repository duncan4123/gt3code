...
(function () {
  ...
  const wMsg = (type, data, transferables) => {
  ...
  const Sqlite3Shell = {
    ...
    dbIsOpfs: function f() {
      return sqlite3.opfs && sqlite3.capi.sqlite3_js_db_uses_vfs(this.dbHandle(), "opfs");
    },
    runMain: function f() {
      ...
      f.argv.rc = wasm.exports.fiddle_main(f.argv.length, f.argv.pArgv);
      if (f.argv.rc) {
  ...
  self.onmessage = function f(ev) {
    ...
    switch (ev.type) {
      case "shellExec":
        Sqlite3Shell.exec(ev.data);
        return;
      case "db-reset":
        Sqlite3Shell.resetDb();
        return;
      case "interrupt":
        Sqlite3Shell.interrupt();
        return;
      ...
      case "db-export": {
        ...
        try {
          ...
          const buffer = sqlite3.capi.sqlite3_js_db_export(Sqlite3Shell.dbHandle());
  ...
  */
  sqlite3InitModule(fiddleModule)
    .then((_sqlite3) => {
      sqlite3 = _sqlite3;
      sqlite3.config.warn(
        "Installing sqlite3 module globally (in Worker)",
