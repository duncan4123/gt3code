...
(function () {
  ...
  const storage = (function (NS /*namespace object in which to store this module*/) {
    ...
    */
    const storageKeyPrefix =
      $storageHolder === $storage /*localStorage or sessionStorage*/
        ? ((NS.config
            ? NS.config.projectCode || NS.config.projectName || NS.config.shortProjectName
    ...
    */
    NS.storage = {
      ...
      get: (k, dflt) =>
        $storageHolder.hasOwnProperty(storageKeyPrefix + k)
          ? $storage.getItem(storageKeyPrefix + k)
          : dflt,
      ...
      */
      storageHelpDescription: function () {
        return {
          localStorage:
            "Browser-local persistent storage with an " +
            "unspecified long-term lifetime (survives closing the browser, " +
            "but maybe not a browser upgrade).",
          sessionStorage: "Storage local to this browser tab, " + "lost if this tab is closed.",
  ...
  */
  const SF =
    ...
    (window.SqliteFiddle /*canonical name*/ = {
      ...
      addMsgHandler: function f(type, callback) {
        ...
        (this._msgMap.hasOwnProperty(type) ? this._msgMap[type] : (this._msgMap[type] = [])).push(
          callback,
        );
        return this;
      },
      ...
      runMsgHandlers: function (msg) {
        const list = this._msgMap.hasOwnProperty(msg.type) ? this._msgMap[msg.type] : false;
        if (!list) {
      ...
      /**
          ...
          */
      resetDb: function () {
        if (window.confirm("Really destroy all content and tables " + "in the (transient) db?")) {
          this.wMsg("db-reset");
        }
        return this;
      },
  ...
  SF.addMsgHandler("sqlite-version", (ev) => {
    ...
    a.innerText = [v.lib, v.srcId.substr(0, 34)].join(" ");
    SF.echo("SQLite version", a.innerText);
  });
  SF.addMsgHandler("wasm-info", (ev) => {
    const v = ev.data;
    SF.e.wasmInfo.innerText =
      "WASM: " + (4 === v.pointerSize ? 32 : 64) + "-bit" + " heap: " + Number(v.heapSize);
  ...
  const EAll = function (/*[element=document,] cssSelector*/) {
    return (arguments.length > 1 ? arguments[0] : document).querySelectorAll(
      arguments[arguments.length - 1],
    );
  };
  /* querySelector() proxy */
  const E = function (/*[element=document,] cssSelector*/) {
    return (arguments.length > 1 ? arguments[0] : document).querySelector(
      arguments[arguments.length - 1],
    );
  };
  ...
  self.onSFLoaded = function () {
    ...
    */
    ...
    SF.addMsgHandler("working", function f(ev) {
      switch (ev.data) {
        case "start":
          /* See notes in preStartWork(). */ return;
        case "end":
    ...
    /* For each checkbox with data-csstgt, set up a handler which
       ...
       E(data-csstgt). */
    EAll("input[type=checkbox][data-csstgt]").forEach(function (e) {
      ...
      e.addEventListener(
        "change",
        function () {
          tgt.classList[this.checked ? "add" : "remove"](cssClass);
        },
        false,
      );
    });
    ...
    EAll("input[type=checkbox][data-config]").forEach(function (e) {
    ...
    const cmdClick = function () {
      SF.dbExec(this.dataset.cmd);
    };
    EAll("button[data-cmd]").forEach((e) => e.addEventListener("click", cmdClick, false));
    ...
    */
    ...
    (function () {
      ...
      xElem.addEventListener("change", function () {
        taInput.value = "-- " + this.selectedOptions[0].innerText + "\n" + this.value;
    ...
    if (window.jQuery && window.jQuery.terminal && SF.e.terminal) {
      ...
      btnToggleView.addEventListener(
        "click",
        function f() {
