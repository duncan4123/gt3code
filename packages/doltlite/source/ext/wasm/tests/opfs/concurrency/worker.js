...
const jsSqlite =
  urlArgs.get("sqlite3.dir") + "/sqlite3.js?opfs-async-proxy-id=" + options.workerName;
importScripts(jsSqlite) /*Sigh - URL args are not propagated this way*/;
//const sqlite3InitModule = (await import(jsSqlite)).default;
globalThis.sqlite3InitModule.__isUnderTest = true /* causes sqlite3.opfs to be retained */;
globalThis.sqlite3InitModule().then(async function (sqlite3) {
  ...
  const finish = () => {
    ...
    if (interval.error) {
      stderr("Ending work at interval #" + interval.count, "due to error:", interval.error);
      wPost("failed", "at interval #" + interval.count, "due to error:", interval.error);
    } else {
  ...
  const run = async function () {
    ...
    while (true) {
      try {
        ...
        if (e instanceof sqlite3.SQLite3Error && sqlite3.capi.SQLITE_BUSY === e.resultCode) {
    ...
    const maxIterations = urlArgs.has("iterations") ? +urlArgs.get("iterations") || 10 : 10;
    stdout(
      "Starting",
      maxIterations,
      "interval-based db updates with delay of",
      interval.delay,
      "ms.",
    );
    const doWork = async () => {
      ...
      try {
        sqlite3.capi.sqlite3_js_retry_busy(
          -1,
          () => {
            db.exec({
    ...
    setTimeout(async function timer() {
      ...
      if (interval.error || maxIterations === interval.count) {
        ...
        const jitter = Math.random() >= 0.5 ? (Math.random() * 100 - Math.random() * 100) | 0 : 0;
