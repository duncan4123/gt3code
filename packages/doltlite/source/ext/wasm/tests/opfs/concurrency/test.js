(async function (self) {
  ...
  options.workerCount = (urlArgsHtml.has("workers") ? +urlArgsHtml.get("workers") : 0) || 3;
  options.opfsVerbose = (urlArgsHtml.has("verbose") ? +urlArgsHtml.get("verbose") : 1) || 0;
  options.interval = (urlArgsHtml.has("interval") ? +urlArgsHtml.get("interval") : 1000) || 1000;
  options.iterations = (urlArgsHtml.has("iterations") ? +urlArgsHtml.get("iterations") : 10) || 10;
  options.unlockAsap = (urlArgsHtml.has("unlock-asap") ? +urlArgsHtml.get("unlock-asap") : 0) || 0;
  ...
  workers.onmessage = function (msg) {
    ...
    switch (msg.type) {
      case "loaded":
        ...
        break;
      case "stdout":
        stdout(prefix, ...msg.payload);
        break;
      case "stderr":
        stderr(prefix, ...msg.payload);
        break;
      case "error":
        stderr(prefix, "ERROR:", ...msg.payload);
        break;
      case "finished": {
      ...
      default:
        logCss("error", "Unhandled message type:", msg);
        break;
    }
  };
  ...
  const optArgs = function (obj) {
    const li = [];
    for (const k of ["interval", "iterations", "unlock-asap", "opfsVerbose", "vfs", "workers"]) {
  ...
  btnGo.addEventListener("click", () => {
    ...
    for (let i = 0; i < options.workerCount; ++i) {
      ...
      workers.push(
        new Worker(
          workers.uri + "&workerId=" + (i + 1) + (i || options.noUnlink ? "" : "&unlink-db"),
        ),
      );
    }
