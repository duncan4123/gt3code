...
1. An sqlite3expert object is created by calling **sqlite3_expert_new()**.
...
1. The sqlite3expert object is configured with one or more SQL statements
   by making one or more calls to **sqlite3_expert_sql()**. Each call may
   specify a single SQL statement, or multiple statements separated by
   semi-colons.

1. Optionally, the **sqlite3_expert_config()** API may be used to
...
1. **sqlite3_expert_analyze()** is called to run the analysis.

1. One or more calls are made to **sqlite3_expert_report()** to extract
   components of the results of the analysis.

1. **sqlite3_expert_destroy()** is called to free all resources.
