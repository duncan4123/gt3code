...
func (cr *CityRuntime) beadReconcileTick(ctx context.Context, result DesiredStateResult, sessionBeads *sessionBeadSnapshot, trace *sessionReconcilerTraceCycle) {
	...
	if workSet == nil {
		workSet = computeWorkSetWithStores(cr.cfg, shellScaleCheck, cityName, cr.cityPath, store, rigStores, sessionBeads, cr.stderr)
	}
	if trace != nil {
...
func (cr *CityRuntime) loadDemandSnapshot(
	...
	if cr.shouldRefreshDemandSnapshot(trigger, configChanged, sessionFingerprint) {
		...
		result.WorkSet = computeWorkSetWithStores(cr.cfg, shellScaleCheck, cr.cityName, cr.cityPath, cr.cityBeadStore(), cr.rigBeadStores(), sessionBeads, cr.stderr)
