...
func (s *closeCountingStore) CloseStore() error {
	s.closes++
	return nil
}

func TestRetireBeadStoresClosesEachResourceOnce(t *testing.T) {
	store := &closeCountingStore{MemStore: beads.NewMemStore()}

	retireBeadStores(map[string]beads.Store{
		"rig1": store,
		"rig2": store,
	}, store)

	if store.closes != 1 {
		t.Fatalf("CloseStore called %d times, want 1", store.closes)
	}
}
