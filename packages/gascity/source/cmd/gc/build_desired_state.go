...
func evaluatePendingPools(
	...
	for j, pw := range pendingPools {
		...
		defaultDemand := pw.defaultDemand
		demandStore := pw.store
		go func(idx int, template, agentName string, agentIndex int, sp scaleParams, dir string, defaultDemand bool, demandStore beads.Store) {
			defer wg.Done()
			sem <- struct{}{}
			defer func() { <-sem }()
			started := time.Now()
			var d int
			var err error
			if defaultDemand && demandStore != nil {
				d, err = evaluateDefaultPoolDemand(template, sp, demandStore)
			} else {
		...
		}(j, template, agentName, agentIndex, sp, pw.poolDir, defaultDemand, demandStore)
	}
	wg.Wait()
...
func defaultScaleCheckForAgent(a *config.Agent) bool {
	return a != nil && strings.TrimSpace(a.ScaleCheck) == ""
}
...
func buildDesiredStateWithSessionBeads(
	...
	for i := range cfg.Agents {
		...
		if backsNamedSession {
			...
			demandStore := workQueryStoreForAgent(cityPath, cfg, &cfg.Agents[i], store, rigStores)
			pendingPools = append(pendingPools, poolEvalWork{agentIdx: i, sp: sp, poolDir: poolDir, store: demandStore, defaultDemand: defaultScaleCheckForAgent(&cfg.Agents[i])})
			continue
		}
		...
		poolDir := agentCommandDir(cityPath, &cfg.Agents[i], cfg.Rigs)
		demandStore := workQueryStoreForAgent(cityPath, cfg, &cfg.Agents[i], store, rigStores)
		pendingPools = append(pendingPools, poolEvalWork{agentIdx: i, sp: sp, poolDir: poolDir, env: controllerQueryRuntimeEnv(cityPath, cfg, &cfg.Agents[i]), store: demandStore, defaultDemand: defaultScaleCheckForAgent(&cfg.Agents[i])})
	}
