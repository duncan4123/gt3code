		...
		go func() {
			...
			if err == nil && item.candidate.session != nil && item.candidate.session.Metadata["session_key"] != "" {
				...
				if store == nil || strings.TrimSpace(item.candidate.session.ID) == "" {
					...
					// This path has the session bead already. Probe by stable bead
					// ID so stores do not have to resolve a runtime session_name
					// through slower metadata/list fallbacks during startup.
					running, err = workerSessionTargetRunningWithConfig(cityPath, store, sp, cfg, item.candidate.session.ID)
				}
				if err != nil || !running {
