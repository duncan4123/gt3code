...
func (cs *controllerState) update(cfg *config.City, sp runtime.Provider) {
	...
	retireBeadStores(oldStores, oldCityStore)
}

type closeStoreResource interface {
	CloseStore() error
}

func retireBeadStores(stores map[string]beads.Store, cityStore beads.Store) {
	seen := map[uintptr]struct{}{}
	for _, store := range stores {
		retireBeadStore(store, seen)
	}
	retireBeadStore(cityStore, seen)
}

func retireBeadStore(store beads.Store, seen map[uintptr]struct{}) {
	if store == nil {
		return
	}
	if key := storeIdentity(store); key != 0 {
		if _, ok := seen[key]; ok {
			return
		}
		seen[key] = struct{}{}
	}
	closer, ok := store.(closeStoreResource)
	if !ok {
		return
	}
	if err := closer.CloseStore(); err != nil {
		log.Printf("api: close retired bead store: %v", err)
	}
}

func storeIdentity(store beads.Store) uintptr {
