...
<Note>
  This page covers the seven failure modes encountered in the 2026-04-26 selfhost UX bug haul
  (umbrella PRD `ga-r8hs`). For installation issues (`gc` not on PATH, missing prerequisites,
  version mismatches), see [Troubleshooting](/getting-started/troubleshooting). For Dolt store
...
<Tip>
  If your symptom does not match any of the above, run `gc doctor --verbose` and consult the
  [installation troubleshooting](/getting-started/troubleshooting) page. If still stuck, file an
  issue (link at the bottom of this page).
</Tip>
...
**Background**: [Pack v1 → v2 migration guide](/guides/migrating-to-pack-vnext) ·
...
[Pack v1 → v2 migration guide](/guides/migrating-to-pack-vnext).
