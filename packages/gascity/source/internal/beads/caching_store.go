...
// CloseStore stops background cache work and closes the backing store when it
// owns closeable resources such as an in-process doltlite connection.
func (c *CachingStore) CloseStore() error {
	c.StopReconciler()
	if closer, ok := c.backing.(interface{ CloseStore() error }); ok {
