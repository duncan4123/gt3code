...
func TestCachingStoreApplyEvent(t *testing.T) {
	...
	// Apply a legacy raw update event.
...
func TestCachingStoreCloseStoreClosesBacking(t *testing.T) {
	t.Parallel()
	backing := &closeableMemStore{MemStore: beads.NewMemStore()}
	cs := beads.NewCachingStoreForTest(backing, nil)

	if err := cs.CloseStore(); err != nil {
		t.Fatalf("CloseStore: %v", err)
	}
	if backing.closes != 1 {
		t.Fatalf("backing closes = %d, want 1", backing.closes)
	}
}
...
func (s *closeableMemStore) CloseStore() error {
	s.closes++
	return nil
}
