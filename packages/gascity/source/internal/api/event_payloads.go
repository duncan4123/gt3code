...
// UnmarshalJSON tolerates historical/doltlite bead payloads whose metadata
// values were encoded as JSON numbers or booleans. The API domain model keeps
// metadata as strings, matching bd's public contract.
func (p *BeadEventPayload) UnmarshalJSON(data []byte) error {
	...
	p.Bead = wire.Bead.Bead
	if wire.Bead.Metadata != nil {
		p.Bead.Metadata = map[string]string(wire.Bead.Metadata)
	}
	return nil
}
