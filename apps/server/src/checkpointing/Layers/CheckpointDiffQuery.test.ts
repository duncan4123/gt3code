...
describe("CheckpointDiffQueryLive", () => {
  ...
  it("computes full-thread diffs from narrow checkpoint context", async () => {
    const projectId = ProjectId.make("project-full");
    const threadId = ThreadId.make("thread-full");
    ...
    const checkpointStore: CheckpointStoreShape = {
      isGitRepository: () => Effect.succeed(true),
      captureCheckpoint: () => Effect.void,
      hasCheckpointRef: () => Effect.die("full-thread diff should not preflight refs"),
      restoreCheckpoint: () => Effect.succeed(true),
      diffCheckpoints: ({ fromCheckpointRef, toCheckpointRef, cwd, ignoreWhitespace }) =>
        Effect.sync(() => {
          diffCheckpointsCalls.push({
      ...
      Layer.provideMerge(
        Layer.succeed(ProjectionSnapshotQuery, {
          ...
          getThreadCheckpointContext: () =>
            Effect.die("full-thread diff should not hydrate all checkpoints"),
