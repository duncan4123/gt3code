...
const codexModelSelection = (model: string, options?: ModelSelection["options"]): ModelSelection =>
  createModelSelection(ProviderInstanceId.make("codex"), model, options);
const claudeModelSelection = (model: string, options?: ModelSelection["options"]): ModelSelection =>
  createModelSelection(ProviderInstanceId.make("claudeAgent"), model, options);
const providerForSelection = (selection: ModelSelection) =>
  selection.instanceId === ProviderInstanceId.make("claudeAgent")
    ? claudeDriverKind
    : codexDriverKind;
...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const modelSelection = input?.threadModelSelection ?? codexModelSelection("gpt-5-codex");
    const startSession = vi.fn((_: unknown, input: unknown) => {
      ...
      const session: ProviderSession = {
        provider: providerForSelection(modelSelection),
  ...
  it("forwards claude effort options through session start and turn send", async () => {
    const harness = await createHarness({
      threadModelSelection: claudeModelSelection("claude-sonnet-4-6"),
    });
    const now = new Date().toISOString();
  ...
  it("forwards claude fast mode options through session start and turn send", async () => {
    const harness = await createHarness({
      threadModelSelection: claudeModelSelection("claude-opus-4-6"),
    });
    const now = new Date().toISOString();
  ...
  it("rejects a first turn when requested provider conflicts with the thread model", async () => {
    const harness = await createHarness({
      threadModelSelection: codexModelSelection("gpt-5-codex"),
    });
    const now = new Date().toISOString();
  ...
  it("does not restart an active session when read-model provider instance metadata is stale", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-stale-provider-instance-1"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-stale-provider-instance-1"),
          role: "user",
          text: "first",
          attachments: [],
        },
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        commandId: CommandId.make("cmd-session-set-stale-provider-instance"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "ready",
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-stale-provider-instance-2"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-stale-provider-instance-2"),
          role: "user",
          text: "second",
          attachments: [],
        },
