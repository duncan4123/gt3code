...
export interface GcBead {
  ...
  /** Parent convoy/epic bead ID. Populated from `parent_id` in the API response. */
...
export interface GcApiClientShape {
  /** Fetch a single bead by ID. Returns null if not found or unavailable. */
  readonly getBead: (id: string) => Effect.Effect<GcBead | null>;

  /** Fetch a convoy by ID. Returns null if not found or unavailable. */
...
/**
 * GcApiClient — Service tag for Gas City REST API access.
 */
