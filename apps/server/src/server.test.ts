...
const testEnvironment = {
  environmentId: "environment-test" as any,
  label: "test",
  platform: { os: "linux", arch: "x64" },
  serverVersion: "test",
  capabilities: { repositoryIdentity: true },
} satisfies ExecutionEnvironmentDescriptor;
const testAuthDescriptor = {
  ...
  Effect.gen(function* () {
    ...
    const routeExtraMocks = Layer.mergeAll(
      ...
      Layer.mock(GcApiClient)({
        ...
        wakeSession: () => Effect.die("unexpected gc wake"),
    ...
    const appLayer = routeLayer.pipe(
      Layer.provide(
        Layer.mock(ServerAuth)({
          getDescriptor: () => Effect.succeed(testAuthDescriptor),
          getSessionState: () =>
            Effect.succeed({
              authenticated: false,
              auth: testAuthDescriptor,
            } satisfies AuthSessionState),
          exchangeBootstrapCredential: () => Effect.die("unexpected auth bootstrap exchange"),
          exchangeBootstrapCredentialForBearerSession: () =>
            Effect.die("unexpected auth bearer bootstrap exchange"),
          issuePairingCredential: () => Effect.die("unexpected auth pairing credential issue"),
          ...
          authenticateHttpRequest: () => Effect.die("unexpected http authentication"),
          ...
          issueWebSocketToken: () => Effect.die("unexpected websocket token issue"),
          issueStartupPairingUrl: () => Effect.succeed("http://127.0.0.1/pair"),
          ...options?.layers?.serverAuth,
        }),
      ),
      Layer.provide(
        Layer.mock(SessionCredentialService)({
          cookieName: testAuthDescriptor.sessionCookieName,
          issue: () => Effect.die("unexpected session issue"),
          verify: () => Effect.die("unexpected session verify"),
          issueWebSocketToken: () => Effect.die("unexpected websocket token issue"),
          verifyWebSocketToken: () => Effect.die("unexpected websocket token verify"),
      ...
      Layer.provide(
        Layer.mock(ServerEnvironment)({
          getEnvironmentId: Effect.succeed("environment-test" as any),
...
const TestHttpLayer = Layer.merge(NodeHttpServer.layerTest, NodeServices.layer);
    ...
    () =>
      Effect.gen(function* () {
        ...
        const liveEvents = Stream.make({
          ...
          sequence: 2,
          type: "ready" as const,
          payload: { at: new Date().toISOString(), environment: testEnvironment },
        });
