...
function fakeGhOutput(stdout: string, stderr = ""): VcsProcess.VcsProcessOutput {
  return {
...
function createGitHubCliWithFakeGh(scenario: FakeGhScenario): GitHubCliShape {
  const execute: GitHubCliShape["execute"] = (input) => {
    ...
    if (args[0] === "pr" && args[1] === "view") {
      ...
      return Effect.succeed(fakeGhOutput(`${JSON.stringify(pullRequest)}\n`));
    }
    ...
    if (args[0] === "repo" && args[1] === "view") {
      ...
      return Effect.succeed(fakeGhOutput("main\n"));
    }
    ...
    createRepository: () =>
      Effect.fail(
        new GitHubCliError({
          ...
          detail: "unused in JjManager tests",
        }),
      ),
  ...
  describe("preparePullRequestThread", () => {
    it.effect("prepares pull request threads in local mode on the PR head branch", () =>
      Effect.gen(function* () {
        ...
        expect(status.refName).toBe("feature/pr-local");
      }),
    );
    ...
    it.effect("creates a new JJ workspace for a same-repo PR and launches setup once", () =>
      Effect.gen(function* () {
        ...
        expect(worktreeStatus.refName).toBe("feature/pr-worktree");
    ...
    it.effect("preserves fork upstream tracking when preparing a JJ PR workspace", () =>
      Effect.gen(function* () {
        ...
        expect(worktreeStatus.refName).toBe("t3code/pr-81/feature/pr-fork");
