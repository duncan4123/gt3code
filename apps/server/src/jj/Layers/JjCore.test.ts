  ...
  describe("commit and push", () => {
    it.effect("commits changes and pushes to a remote", () =>
      Effect.gen(function* () {
        ...
        expect(statusAfterCommit.refName).toBe("main");
  ...
  describe("createBranch", () => {
    it.effect("creates a bookmark at the working copy revision", () =>
      Effect.gen(function* () {
        ...
        const status = yield* jjCore.statusDetails(repoDir);
        expect(status.refName).toBe("feat/new");
      }),
    );
  });
  ...
  describe("createWorktree", () => {
    it.effect("creates distinct jj workspaces for distinct temporary thread branches", () =>
      Effect.gen(function* () {
        ...
        expect(firstStatus.refName).toBe("t3code/11111111");
        expect(secondStatus.refName).toBe("t3code/22222222");
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(worktreeStatus.refName).toBe(syntheticBranch);
