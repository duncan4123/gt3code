...
const mapGitHubCliError = (operation: string, error: GitHubCliError): GitManagerError =>
  jjManagerError(operation, error.message, error);
...
export const makeJjManager = Effect.fn("makeJjManager")(function* () {
  ...
  const status: JjManagerShape["status"] = Effect.fn("status")(function* (input) {
    ...
    if (!current.isRepo || !currentBranch || !current.hasPrimaryRemote) {
      return current;
    }

    const headSelector = yield* resolveHeadSelectorBranch(input.cwd, currentBranch);
