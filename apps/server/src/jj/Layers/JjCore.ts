...
export const makeJjCore = Effect.fn("makeJjCore")(function* () {
  ...
  const readWorkspaceRefs = (
    ...
    runJjStdout("JjCore.readWorkspaceRefs", cwd, [
      ...
      "list",
      "-T",
      'name ++ "\\t" ++ root ++ "\\t" ++ target.change_id().short() ++ "\\t" ++ target.commit_id().short() ++ "\\t" ++ target.bookmarks() ++ "\\n"',
    ]).pipe(
      Effect.map(parseWorkspaceListEntries),
  ...
  const statusDetails: JjCoreShape["statusDetails"] = Effect.fn("statusDetails")(function* (cwd) {
    ...
    if (!localCommit || !currentCommit) {
      return {
        ...
        hasOriginRemote,
        isDefaultRef: branch !== null && branch === defaultBranch,
  ...
  const listBranches: JjCoreShape["listBranches"] = Effect.fn("listBranches")(function* (input) {
    ...
    const workspacesByBookmark = new Map<string, ParsedWorkspaceRef>();
    for (const workspace of workspaceRefs) {
      for (const bookmark of workspace.bookmarks) {
        if (!workspacesByBookmark.has(bookmark)) {
          workspacesByBookmark.set(bookmark, workspace);
        }
      }
    }
          ...
          ({
            ...
            ...(workspacesByBookmark.get(bookmark.name)
              ? {
                  ...
                  changeId: workspacesByBookmark.get(bookmark.name)?.changeId,
                  commitId: workspacesByBookmark.get(bookmark.name)?.commitId,
                }
              : {}),
          }) satisfies GitBranch,
      )
    ...
    const adjustedLocalBranches: GitBranch[] = localBranches.map((branch) => {
      const worktreePath =
        branch.name === currentStatus.branch ? workspaceRoot : (branch.worktreePath ?? null);

      return {
        name: branch.name,
        kind: branch.kind,
        ...(branch.workspaceName !== undefined ? { workspaceName: branch.workspaceName } : {}),
        ...(branch.changeId !== undefined ? { changeId: branch.changeId } : {}),
        ...(branch.commitId !== undefined ? { commitId: branch.commitId } : {}),
    ...
    const bookmarkBackedWorkspaceNames = new Set(
      workspaceRefs
        .filter((workspace) => workspace.bookmarks.length > 0)
        .map((workspace) => workspace.name),
    );
    const workspaceRefsWithoutBookmark: GitBranch[] = workspaceRefs
      .filter((workspace) => !bookmarkBackedWorkspaceNames.has(workspace.name))
      .map(
        (workspace) =>
          ({
            ...
            current: canonicalizePath(workspace.root) === workspaceRoot,
            isRemote: false,
            isDefault: false,
    ...
    const paginated = paginateBranches({
      branches: filterBranchesForListQuery(
        dedupeRemoteBranchesWithLocalMatches([
          ...adjustedLocalBranches,
          ...workspaceRefsWithoutBookmark,
          ...remoteBranches,
        ]),
        input.query,
      ),
    ...
    return {
      ...
      refs: [...paginated.branches],
      isRepo: true,
      hasOriginRemote: currentStatus.hasOriginRemote,
      hasPrimaryRemote: currentStatus.hasOriginRemote,
  ...
  const createWorktree: JjCoreShape["createWorktree"] = Effect.fn("createWorktree")(
    function* (input) {
      const baseBranch = input.branch ?? input.refName;
      if (!baseBranch) {
        return yield* commandError(
          "JjCore.createWorktree",
          input.cwd,
          "jj workspace add",
          "Cannot create a workspace without a ref name.",
        );
      }
      const targetBranch = input.newBranch ?? input.newRefName ?? baseBranch;
      if (input.newBranch ?? input.newRefName) {
        yield* runJjCommand({
          operation: "JjCore.createWorktree.createBookmark",
          cwd: input.cwd,
          args: ["bookmark", "create", targetBranch, "--revision", baseBranch],
        });
      }
  ...
  const fetchRemoteTrackingBranch: JjCoreShape["fetchRemoteTrackingBranch"] = (input) =>
    runJjCommand({
      operation: "JjCore.fetchRemoteTrackingBranch",
      cwd: input.cwd,
      args: ["git", "fetch", "--remote", input.remoteName, "--branch", input.remoteBranch],
      timeoutMs: 30_000,
    }).pipe(Effect.asVoid);
  ...
  const createBranch: JjCoreShape["createBranch"] = Effect.fn("createBranch")(function* (input) {
    const branch = "branch" in input ? input.branch : input.refName;
    yield* runJjCommand({
      operation: "JjCore.createBranch",
      cwd: input.cwd,
      args: ["bookmark", "create", branch, "--revision", "@"],
    });
    ...
    yield* recordWorkspaceBranch(repoDir, workspaceRoot, branch).pipe(
      Effect.catch(() => Effect.void),
    );
    return "refName" in input ? { refName: branch } : undefined;
  });
