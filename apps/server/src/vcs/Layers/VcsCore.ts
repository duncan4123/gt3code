...
export const VcsCoreLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const routed = {
      ...
      resolvePrimaryRemoteName: (cwd) => selectCore(cwd).resolvePrimaryRemoteName(cwd),
      fetchRemoteBranch: (input) => selectCore(input.cwd).fetchRemoteBranch(input),
      fetchRemoteTrackingBranch: (input) => selectCore(input.cwd).fetchRemoteTrackingBranch(input),
