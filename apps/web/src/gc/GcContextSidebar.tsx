...
function RuntimeMcpServers({ runtime }: { runtime: GcRuntimeProjection | undefined }) {
...
function RuntimeSkills({ runtime }: { runtime: GcRuntimeProjection | undefined }) {
...
const GcContextSidebar = memo(function GcContextSidebar({
  ...
  const hookFormula = formula?.name ?? gcMeta.formula;
  const hookMolecule = gcMeta.molecule;
  const runtimeEnvRows = useMemo(() => getRuntimeEnvRows(gcMeta.sessionEnv), [gcMeta.sessionEnv]);
  const runtime = (threadContext as { runtime?: GcRuntimeProjection } | null | undefined)?.runtime;
  const beadMetadata = undefined;
  ...
  const sessionWorkDir = gcMeta.sessionEnv?.GC_DIR;
  const rigRoot = gcMeta.sessionEnv?.GC_RIG_ROOT;
    ...
    <>
      ...
      <SidebarContent className="gap-0">
        <SectionCard title="Agent">
          ...
          <ContextRow label="Template" value={gcMeta.sessionEnv?.GC_TEMPLATE} />
          <ContextRow label="Model" value={undefined} />
        ...
        {(hookBeadId || bead) && (
          <SectionCard title="Hooked Work">
            ...
            <ContextRow label="Formula Ref" value={hookFormula} mono />
