...
## MCP recovery — MANDATORY

If a `mcp__context_mode__*` tool stops working because the context-mode server died or disconnected, run:

```bash
context-mode-doltlite restart
```

Run it from the project root, then retry the MCP tool call once. The command kills the recorded stdio server for the current project and clears stale PID state so Codex can spawn a fresh server on the next MCP call.
