...
`);try{if(!y(s))throw new Error(`patch-doltlite.mjs not found at ${s}`);h(`"${process.execPath}" "${s}"`,{cwd:E(e,".."),stdio:["ignore","pipe","pipe"],timeout:12e4}),delete r.cache[r.resolve("better-sqlite3")],c=r("better-sqlite3"),process.stderr.write(`[context-mode] better-sqlite3 rebuilt successfully
