import{createRequire as g}from"node:module";import{execSync as h}from"node:child_process";import{existsSync as y,unlinkSync as R}from"node:fs";import{tmpdir as S}from"node:os";import{join as E,dirname as v}from"node:path";import{fileURLToPath as L}from"node:url";var m=class{#t;constructor(t){this.#t=t}pragma(t){let s=this.#t.prepare(`PRAGMA ${t}`).all();if(!s||s.length===0)return;if(s.length>1)return s;let i=Object.values(s[0]);return i.length===1?i[0]:s[0]}exec(t){let e="",s=null;for(let o=0;o<t.length;o++){let a=t[o];if(s)e+=a,a===s&&(s=null);else if(a==="'"||a==='"')e+=a,s=a;else if(a===";"){let u=e.trim();u&&this.#t.prepare(u).run(),e=""}else e+=a}let i=e.trim();return i&&this.#t.prepare(i).run(),this}prepare(t){let e=this.#t.prepare(t);return{run:(...s)=>e.run(...s),get:(...s)=>{let i=e.get(...s);return i===null?void 0:i},all:(...s)=>e.all(...s),iterate:(...s)=>e.iterate(...s)}}transaction(t){return this.#t.transaction(t)}close(){this.#t.close()}},c=null;function f(){if(!c){let r=g(import.meta.url);if(globalThis.Bun){let t=r(["bun","sqlite"].join(":")).Database;c=function(s,i){let o=new t(s,{readonly:i?.readonly,create:!0});return new m(o)}}else try{c=r("better-sqlite3")}catch(t){if(t?.message?.includes("NODE_MODULE_VERSION")||t?.message?.includes("was compiled against")||t?.code==="ERR_DLOPEN_FAILED"){let e=v(L(import.meta.url)),s=E(e,"..","scripts","patch-doltlite.mjs");process.stderr.write(`[context-mode] ABI mismatch detected (Node ${process.version}), rebuilding better-sqlite3 with doltlite...
